...
## TodoList (v1 — completed 2026-08-30)
...
- [x] Cache: `deconflict cache clean [--dry-run|--all|--older-than-days]` prunes stale caches; `clean_cache()` in cache.py
- [x] Cache: invalidate active cache after a real resolve (`_invalidate_cache`), never on `--dry-run`
- [x] Cache: `pytest_sessionfinish` hook in tests/conftest.py prunes shared default cache of dead-dir entries
- [x] Fix pre-existing `move_to_backup` ENAMETOOLONG bug (unbounded collision re-prefix) → bounded `_unique_target`
...
- [x] Fix bare `deconflict` (no args) → interactive resolve (`invoke_without_command=True` already in place)
- [x] kind-aware actions: per-kind tool matrix; `[file_types]` config overrides; `(?)tools` found/missing + hints
- [x] `(v)iew` opens both files in suggested app; kdbx keepass merge recipe; CLI pauses on Enter
- [x] `(q)uit` aborts after N groups, prints "aborted after N of total"
- [x] overview table: size + vs-base delta, metadata verdict + detail, mtime, role (`#0`/`#N`) columns
- [x] `(?)tools` shows kind → view tool + found/missing + install hints
- [x] `(m)etadata` menu option; `media.metadata_fields()` per kind; common (dimmed) + diff rows
- [x] numbered keep actions for multiple copies (`_keep_by_number`)
- [x] `_truncate_meta` (≤70 chars) for long metadata values
- [x] metadata diff table headers show `#N` + truncated filename, `overflow="ellipsis"`
- [x] CliRunner test for bare-mode interactive (skip all), quit, `?`-tools, table-flags, no-dirs guidance
- [x] `move_to_backup` fast path: atomic `os.replace` same-fs, EXDEV copy→verify→remove
- [x] launchers print full argv with paths (quoted spaces); regression test
- [x] metadata detail table always has size/created/modified + per-kind sections; identical attr rows dimmed
- [x] ExifTool enrichment for audio+image metadata (`_exiftool_fields`, fallback); unified `_content_fields` extractor
- [x] scan/analyze progress hooks + "scanning/cached" console output (`Engine.cache_used`, `_report_scan`)
- [x] `(?)tools` footer prints effective config path + which TOML sections to edit
- [x] `XDG_CONFIG_HOME`/`XDG_CACHE_HOME` honored (config + cache dir)
- [x] view/edit collapse when same exe; `(o)pen` rename for office
- [x] `GroupAnalysis.meta_fields` computed once in analyze; `(m)etadata` always offered; embedded cover art (APIC/PIC) excluded from summary fields
- [x] unified `_content_fields` so overview verdict and `(m)` rows can't disagree (+ regression test)
- [x] office metadata reads docProps/core.xml + stripped text (no XML soup)
- [x] char-based diff via `SequenceMatcher` (`_hl_runs` + `_print_rich_line`)
- [x] final summary counts resolved vs skipped (`resolved N group(s)`, `(M skipped)`)
- [x] tool-actions matrix: VIEW/EDIT/DIFF/VIEW_META/EDIT_META, KIND_*TOOL tables, `[file_types_edit]`, `(x)edit-meta`
- [x] `(h)keep-both` implemented (renames copies to `<base> (copy N).ext`, non-destructive, dry-run safe)
- [x] office `(d)iff` = extracted-text terminal diff (soffice `--compare` removed after build error)
- [x] `Launcher.tools_for(kind)` only for this kind's actions; `deconflict config tools` subcommand
- [x] meld-specific text action dropped; `(d)iff` = inline console diff; `(m)etadata` owns hotkey `m`
- [x] Fix `deconflict config` MarkupError crash (escape() entire rendered line)
- [x] Fix latches: `metadata table header truncation` start-only → also keep conflict-pattern tail (`start…tail`); `full paths` in group print (see next batch — moved to 2026-09-01 batch)

## Next batch (2026-09-01) — grouped, dependency-ordered
New TODO items added by user on main. Ordering rationale: CLI essentials first (cheap, independent),
then table rendering polish (contained), then the interactive-flow revamp (biggest UX change, feeds
recommendation), then launcher/flow fixes, then auto-recommended (depends on recommendation), then
...
### Next batch (2026-09-02) — items from user session

- [x] **scan progress bar**: replaced the indeterminate spinner (`total=None`) with a live counter + current-file description (`scanning (N files): <file>`), then a `scanned N files` 100% line at the end. Bar itself holds at 0/1 (no known total up front — a pre-count pass would double the walk cost), but the live counter + filename gives visible forward movement.
- [x] **recommendation display — duplicate output**: the `recommend: keep X` line was printed twice (once plain, once with `(Enter)` markup). Now printed ONCE — only via the `(Enter) keep #N 'file' (newest)` menu line in `_menu_text`; the standalone print in `_show_group` was removed.
- [x] **recommendation display — formatting**: labels now carry `#N` (`#1 'file' (newest)` / `#0 base 'file' (newest)` / `#0 base (all files identical)`). The single remaining line is `(Enter) keep #N ...` styled bold cyan.
- [x] **recommendation display — TUI**: TUI now has a "keep recommended" action with hotkey **`r`** (NOT Enter, which selects files in the left pane). A `RECOMMENDED` ActionEntry is prepended to the action bar when a recommendation exists.
- [x] **recommendation logic — mp3 (Fröhlicher Kreis)**: `recommend()` moved to the UI-free `analyze.py` and now rejects a newer copy for NON-text kinds when it is **strictly smaller** than the base (possible metadata/content loss). The Fröhlicher Kreis copy (3603991 < 3604984) is no longer recommended. Same for Vorlage_Transkription.docx and Übersicht.ods (newer but smaller).
- [x] **recommendation logic — README.md**: `_text_superset` now strips blank/whitespace lines before comparing, so an effectively-empty base (1-byte space) is treated as empty and the copy adding real content is correctly recommended.
